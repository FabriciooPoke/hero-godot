...
- **`canvas_items + expand` deixa qualquer tamanho fixo (raio, offset) em unidades de canvas inconsistente entre retrato e paisagem** — só o eixo que "aperta" contra a resolução base (1080x1920) mantém 1 unidade ≈ 1px físico; o outro eixo "sobra" livremente. Em paisagem, quem aperta é a altura (1920), o que deixa a densidade de pixel bem menor que em retrato (largura 1080 aperta). Um raio de botão fixo fica ~45% menor fisicamente em paisagem. HUD.gd resolve isso reescalando por `minf(viewport.x, viewport.y) / 1080` a cada resize — qualquer elemento touch NOVO precisa entrar nessa lista (`_preparar_controles_responsivos`) ou vai sofrer o mesmo bug.
